...
CUDA_VISIBLE_DEVICES=$gpu python train_reinforce_gym.py --eval-n-runs 1 --demo --monitor --load $model --outdir $outdir/demo/best --gpu 0
model=$(find $outdir/train -name "*_finish")
CUDA_VISIBLE_DEVICES=$gpu python train_reinforce_gym.py --eval-n-runs 1 --demo --monitor --load $model --outdir $outdir/demo/finish --gpu 0
