...
def worker(remote, env_fn):
    ...
    try:
        while True:
            ...
            elif cmd == "set_road_num":
                if type(data) == int or type(data) == float:
                    env.road_num = data
                remote.send(env.road_num)
            else:
...
"""
    ...
    def set_or_get_road_num(self, road_nums):
        self._assert_not_closed()
        for remote, road_num in zip(self.remotes, road_nums):
            remote.send(("set_road_num", road_num))
        results = [remote.recv() for remote in self.remotes]
        return results
