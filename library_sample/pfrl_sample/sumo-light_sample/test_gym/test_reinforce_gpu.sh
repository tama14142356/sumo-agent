...
tempdir=$(mktemp -d)
outdir=$project_root/results/sumo-light/prfl_result/reinforce/$(basename $tempdir)
...
CUDA_VISIBLE_DEVICES=$gpu python train_reinforce_gym.py --demo --monitor --load $model --outdir $outdir/demo --gpu 0
